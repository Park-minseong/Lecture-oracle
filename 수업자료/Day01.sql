...
8. 테이블의 데이터를 조회하는 SELECT FROM 구문
    --지정한 컬럼의 데이터만 조회
    - SELECT 조회할 데이터의 컬럼명 FROM 조회할 테이블명
    --모든 컬럼의 데이터 조회
    - SELECT * FROM 조회할 테이블명
    -쿼리를 구현할 때는 엔터 값과 들여쓰기를 적절히 활용하여
     쿼리 분석이 용이하도록 구현한다
    -조회 쿼리는 테이블정보가 변경되지 않기 떄문에 COMMIT이 불필요
  
9. 조회하는 컬럼명이나 테이블에 별칭 붙이기
    - 조회하는 컬럼명 또는 테이블 (AS: 생략가능) 별칭

10. 데이터값이 NULL값인지를 검사하는 NVL 구문
    - NVL(조회할 컬럼, null값이면 대체해서 표출한 구문)
...
15. 데이터 값이 null인지 아닌지를 검사하는 IS NULL
    - SELECT * FROM 테이블명
        WHERE null인지 체크할 컬럼 IS NULL
    - IS NULL: 데이터값이 null인 데이터 반환
...
17. 다중 조건을 걸어주는 OR 구문
    - SELECT * FROM 테이블명
        WHERE 조회조건1
           ...
           OR 조회조건3
                   ...;
    - 조회 조건을 하나만 충족하는 데이터 모두 표출

18. 문자열을 검색하는 데 사용하는 LIKE 구문
    - SELECT * FROM 테이블명
        WHERE 문자열 비교할 컬럼명 LIKE 검색할 문자열;
    - '_': 한개의 문자열 의미
           글자수가 하나로 된 데이터만 검색
    - '%': 문자열을 대체(없는 경우도 포함)
    - LIKE '김%': '김'으로 시작되는 데이터 모두 검색
    - LIKE '%경': '경'으로 끝나는 데이터 모두 검색
    - LIKE '화_': '화'로 시작하고 두 글자인 데이터 모두 검색
    - LIKE '_화': '화'로 끝나고 두 글자인 데이터 모두 검색
    - LIKE '_동_': 가운데 글자가 '동'이고 세글자인 데이터 모두검색
